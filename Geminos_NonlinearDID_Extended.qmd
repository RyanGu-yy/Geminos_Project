...
tau <- tau <- 
  1.5 +
  0 * sin(age / 2) +
  0 * (genre_num == 1) +   # Action: higher effect
  0 * (genre_num == 2)     # Drama: lower effect
...
m0_stress <-
  ...
  1.0 * ((age_s > 26) & (major_release_s == "tech")) +rnorm(n_stress, 0, 1.5)
...
## Exploratory Plots (Stress-Test Scenario)

We use the same exploratory comparisons as in the first DGP so the second scenario can be read side by side.

```{r scenario-b-plots, fig.width=10, fig.height=4}
p1_stress <- ggplot(stress_data, aes(x = age, y = Y_post, color = factor(D))) +
  geom_point(alpha = 0.15, size = 0.8) +
  geom_smooth(se = FALSE, linewidth = 1.2) +
  scale_color_manual(values = c("Control" = "#4477AA", "Treated" = "#EE6677")) +
  labs(
    title = "Post-outcome vs Age",
    color = "Group"
  ) +
  theme_minimal()

p2_stress <- ggplot(stress_data, aes(x = age, y = diff, color = factor(D))) +
  geom_point(alpha = 0.15, size = 0.8) +
  geom_smooth(se = FALSE, linewidth = 1.2) +
  scale_color_manual(values = c("Control" = "#4477AA", "Treated" = "#EE6677")) +
  labs(
    title = "First Difference (Y_post - Y_pre2) vs Age",
    color = "Group"
  ) +
  theme_minimal()

p3_stress <- ggplot(stress_data, aes(x = age, y = tau)) +
  geom_point(alpha = 0.2, size = 0.8, color = "#228833") +
  geom_smooth(se = FALSE, color = "#228833", linewidth = 1.2) +
  labs(title = "True Individual Treatment Effect τ_i vs Age") +
  theme_minimal()
